...
const priorityMap: Record<string, string> = {
  "Cân bằng": "BALANCED",
  "Nhiều chất đạm": "HIGH_PROTEIN",
  "Nhiều tinh bột": "HIGH_CARBS",
  "Nhiều chất béo": "HIGH_FAT",
  "Nhiều chất xơ": "HIGH_FIBER",
};
...
export default function SuggestionsPage() {
  ...
  const [dailyLog, setDailyLog] = useState<DashboardDailyLogResponse | null>(
    null,
  );
  ...
  const [selectedFoodPreview, setSelectedFoodPreview] =
  ...
  const fetchRecommendations = async (meal: string, priority: string) => {
    ...
    try {
      ...
      if (errMsg.includes("not available") || errMsg.includes("ECONNREFUSED")) {
        ...
        toast.warning(
          "Dịch vụ gợi ý đang không khả dụng. Vui lòng thử lại sau.",
        );
      } else {
  ...
  const handleFoodClick = async (food: FoodItem) => {
    setSelectedFoodPreview(food);
    setSelectedFood(null);
    setDetailOpen(true);
    setDetailLoading(true);

    try {
      const res = await foodService.getFoodById(food.id);
      setSelectedFood(res.data ?? null);
    } catch {
  ...
  return (
    <div className="h-screen w-full flex flex-col pt-20 md:pt-28 px-4 sm:px-6 md:px-10 pb-4 md:pb-6 overflow-hidden">
      ...
      <FoodDetailDialog
        ...
        foodPreview={selectedFoodPreview}
      />
    </div>
  );
}
